...
function generate_UserID(callback) {
  const query =
    "SELECT MAX(CONVERT(user_id, SIGNED)) AS max_id FROM account_details";
          ...
          connection.query(
            ...
            function (error, results2, fields) {
              ...
              connection.commit(function (err) {
                ...
                res
                  .status(200)
                  .json({ message: "Employee account created successfully" });
              });
            }
          );
        }
  ...
  generate_UserID((err, user_id) => {
    ...
    connection.beginTransaction(function (err) {
      ...
      const insertAccountQuery =
        "INSERT INTO account_details (user_id, password, email, type) VALUES (?, ?, ?, ?)";
      const insertIdentityQuery =
        "INSERT INTO identity (adhar_id, phone_no, location) VALUES (?, ?, ?)";
      const insertOwnerQuery =
        "INSERT INTO owner (owner_id, owner_name, owner_age, adhar_no, user_id) VALUES (?, ?, ?, ?, ?)";
        ...
        function (error, results1, fields) {
          ...
          connection.query(
            ...
            function (error, results2, fields) {
              ...
              connection.query(
                insertOwnerQuery,
                [user_id, owner_name, owner_age, adhar_id, user_id],
                function (error, results3, fields) {
                  ...
                  connection.commit(function (err) {
                    ...
                    res
                      .status(200)
                      .json({ message: "Owner account created successfully" });
                  });
                }
              );
            }
...
const addHouseOwner = (req, res, next) => {
  ...
  connection.query(getUserIdQuery, [email], function (error, results, fields) {
    ...
    const updateHouseOwnerQuery =
      "UPDATE house SET owner_id = ? WHERE house_no = ?";

    connection.query(
      updateHouseOwnerQuery,
      [user_id, house_no],
